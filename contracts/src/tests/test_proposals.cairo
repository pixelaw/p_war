...
use pwar::constants::{DEFAULT_AREA, GAME_DURATION, PROPOSAL_DURATION};
use pwar::tests::utils::{deploy_pwar};
use pwar::{
    models::{PaletteColors, guilds::{Guild}},
    systems::{
        actions::{IActionsDispatcherTrait, pwar_actions},
        guilds::{IGuildDispatcherTrait, guild_actions},
    },
};
use starknet::{
...
fn test_add_color() {
    ...
    let (pwar_actions, propose_action, voting_action, _guild) = deploy_pwar(ref world);
    ...
    let index = propose_action
        .create_proposal(game_id: id, proposal_type: 1, target_args_1: NEW_COLOR, target_args_2: 0);
